...
const generatePackageJson = (options) => {
    ...
    substituteModules(bundlePkg.dependencies, moduleName => moduleName.startsWith('dvtemplate') || moduleName.startsWith('@datavillage-me/'));
    ...
    return [
        {
            ...
            plugins: [
                 ...
                 */
                resolve({
                    resolveOnly: ['dvtemplate-core', 'dvtemplate-engine', '@datavillage-me/api']
                }),
                copy({
                    targets: [
                        { src: './static/*', dest: 'dist/standalone/static' },
                        { src: '../yarn.lock', dest: 'dist/standalone' },
                        { src: '../dvtemplate-frontend/dist/*', dest: 'dist/standalone/static' }
                    ]
                }),
                typescript(
                    {
